...
RSpec.describe ExportRun do
  ...
  %w[csv xlsx json].each do |format|
    it "is valid with format #{format}" do
      run = described_class.new(user: user, format: format)
      expect(run).to be_valid
    end
  end

  it 'is invalid without a user' do
    run = described_class.new(format: 'csv')
    expect(run).not_to be_valid
    expect(run.errors[:user]).to include('must exist')
  end

  it 'assigns a download token on creation' do
    run = described_class.create!(user: user, format: 'csv')
    expect(run.download_token).to be_present
  end

  it 'does not overwrite a pre-set download token' do
    run = described_class.create!(user: user, format: 'csv', download_token: 'mytoken')
  ...
  describe '.recent_first' do
    it 'orders records newest first' do
      old_run = described_class.create!(user: user, format: 'csv', created_at: 2.days.ago)
      new_run = described_class.create!(user: user, format: 'json')

      expect(described_class.recent_first.first).to eq(new_run)
      expect(described_class.recent_first.last).to eq(old_run)
    end
  end
end
