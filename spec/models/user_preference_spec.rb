...
RSpec.describe UserPreference, type: :model do
  ...
  it 'is valid with valid attributes' do
    expect(UserPreference.new(valid_attrs)).to be_valid
  end
  ...
  describe 'generator_defaults validation' do
    it 'rejects length below 8' do
      pref = UserPreference.new(valid_attrs.merge(
                                  generator_defaults: { length: 4, symbols: true, numbers: true, uppercase: true }
                                ))
      expect(pref).not_to be_valid
      expect(pref.errors[:generator_defaults]).to include(match(/length must be/))
    end
  ...
  describe 'reminder_defaults validation' do
    it 'rejects lead_days below 0' do
      pref = UserPreference.new(valid_attrs.merge(reminder_defaults: { lead_days: -1, repeat: 'monthly' }))
      expect(pref).not_to be_valid
    end

    it 'rejects lead_days above 365' do
      pref = UserPreference.new(valid_attrs.merge(reminder_defaults: { lead_days: 400, repeat: 'monthly' }))
      expect(pref).not_to be_valid
    end

    it 'rejects an invalid repeat value' do
      pref = UserPreference.new(valid_attrs.merge(reminder_defaults: { lead_days: 7, repeat: 'yearly' }))
      expect(pref).not_to be_valid
      expect(pref.errors[:reminder_defaults]).to include(match(/repeat must be one of/))
    end

    it 'accepts all canonical repeat values' do
      UserPreference::REPEAT_VALUES.each do |repeat|
        pref = UserPreference.new(valid_attrs.merge(reminder_defaults: { lead_days: 7, repeat: repeat }))
